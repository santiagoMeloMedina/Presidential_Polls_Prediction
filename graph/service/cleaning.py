...
from constant.data import COUPLE, INDIVIDUAL, COLS, DIVISION, CASTTYPE, COLS_GRAPH, DIVISION_GRAPH
...
def selectColumnsGraph(df):
    """ This function returns the subsets of the dataframe with the structure specified for 
        each candidate """
    df = castingData(df)
    general = df.select(COLS_GRAPH)
    incumbent = general.select(DIVISION_GRAPH["INC"])
    challenger = general.select(DIVISION_GRAPH["CHAL"])
    return incumbent, challenger
